...
public:
    VelocityControlNode()
        : Node("velocity_control_node"),
          ...
          serial_fd(-1),
          serial_delay_ms_(50) {    // NEW: Added configurable delay variable (default 50ms)
        ...
        // NEW: Declare ROS2 parameter for configurable serial delay
        this->declare_parameter("serial_delay_ms", 50);
...
private:
    ...
    void initializeSerial() {
        ...
        if (tcsetattr(serial_fd, TCSANOW, &tty) != 0) {
            ...
            usleep(2000000);
            // MODIFIED: Added delay info to initialization log
            RCLCPP_INFO(this->get_logger(), "Serial port initialized successfully with %dms delay", serial_delay_ms_);
        }
    }
    ...
    // Serial
    int serial_fd;
    int serial_delay_ms_;    // NEW: Configurable delay in milliseconds
...
2. NEW MEMBER VARIABLE:
   - int serial_delay_ms_;  // Configurable delay in milliseconds (default: 50ms)
...
# Custom 100ms delay:
ros2 run your_package velocity_control_node --ros-args -p serial_delay_ms:=100

# Faster 25ms delay:
ros2 run your_package velocity_control_node --ros-args -p serial_delay_ms:=25
*/
